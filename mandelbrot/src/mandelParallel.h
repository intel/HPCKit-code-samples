...
private:
	...
	static std::complex<float> complex_add( std::complex<float> c0, std::complex<float> c1){
...
public:
	...
	MandelParallel(unsigned int device_index) : b(range<2>(row_count, col_count)),
  	...
  	void Evaluate(SYCLObj syclObj) {
		...
		queue_event = myQueue.submit([&](handler &cgh) {
    		...
    		cgh.parallel_for<class manSYCL>(range<2>(row_count, col_count), [=](id<2> index) {
				int i = int(index[0]); int j = int(index[1]);

				kb[index] = point(std::complex<float>(scaleRow(i), scaleCol(j)), max_iterations);
			});
		});

#ifdef GET_KERNEL_TIME
		queue_event.wait();
		startk = queue_event.template get_profiling_info<cl::sycl::info::event_profiling::command_start>();
		endk = queue_event.template get_profiling_info<cl::sycl::info::event_profiling::command_end>();
	...
	{
		if ((row_max != row_count) || (col_max != col_count))
			return -1; 
		...
		for (int i = 0; i < row_count; i++) {
			for (int j = 0; j < col_count; j++) {
				if (pBufferToCompare[i][j] != kb[i][j]) 
					diff++;
			}
