...
void Usage(string program_name) {
  cout << " Incorrect number of parameters "
       << "\n";
  cout << " Usage: ";
  cout << program_name << " <Number of Iterations> "
       << " <Seed for RNG> "
       << "\n"
       << "\n";
}
...
void ParticleMotion(queue& q, size_t seed, float* particle_X, float* particle_Y,
  ...
  mkl::rng::philox4x32x10 engine(q,
                                 seed);  // basic random number generator object
  mkl::rng::gaussian<float, mkl::rng::gaussian_method::box_muller2> distr(
      0.0, .03);  //  distribution object
