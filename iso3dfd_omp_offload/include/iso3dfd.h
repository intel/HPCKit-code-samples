...
#define STENCIL_LOOKUP(ptr_prev, ix, ir, n1, dimn1n2) \
		(coeff[ir] * ((ptr_prev[ix + ir] + ptr_prev[ix - ir]) + (ptr_prev[ix + ir * n1] + ptr_prev[ix - ir * n1]) + (ptr_prev[ix + ir * dimn1n2] + ptr_prev[ix - ir * dimn1n2])))
...
bool verifyResults(float *, float *, float *, float *,
