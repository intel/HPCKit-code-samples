 ...
 * Inner most loop order is changed from CPU OpenMP version to represent work-items in X-Y plane.
 * And each work-item traverses the Z-plane
 */
void inline iso_3dfd_it(float *ptr_next_base, float *ptr_prev_base, float *ptr_vel_base,
...
// which is enforced here for demonstrate the default version. 
 ...
 * And each work-item traverses the Z-plane
 */
void inline iso_3dfd_it_tiled(float *ptr_next_base, float *ptr_prev_base, float *ptr_vel_base,
	float *coeff, const int n1, const int n2, const int n3,
	const int n1_Tblock, const int n2_Tblock, const int n3_Tblock) {
	int dimn1n2 = n1 * n2;
	int size = n3 * dimn1n2;
...
#pragma omp target data map(ptr_next[0:size], ptr_prev[0:size]) \
                map(ptr_vel[0:size], coeff[0:9], n1, n2, n3, n1_Tblock, n2_Tblock, n3_Tblock)
        for (int it = 0; it<nreps; it += 1) {
                ...
                iso_3dfd_it(ptr_next, ptr_prev, ptr_vel, coeff,
                        n1, n2, n3, n1, n2, n3);
#else
                iso_3dfd_it_tiled(ptr_next, ptr_prev, ptr_vel, coeff,
...
int main(int argc, char* argv[]) {
	...
	std::cout << "Tile Sizes: "<< n1_Tblock << " " << n2_Tblock << " " << n3_Tblock << std::endl;
	std::cout << "Memory Usage (MBytes): "<< ((3 * nsize * sizeof(float))/(1024*1024)) << std::endl;
	...
	error = verifyResults(next_base, prev_base, vel_base, coeff, n1, n2, n3, nIterations, n1_Tblock, n2_Tblock, n3_Tblock);
#endif
	delete[] prev_base;
	delete[] next_base;
	delete[] vel_base;
