...
bool iso_3dfd_device(cl::sycl::queue& q, float* ptr_next, float* ptr_prev, float* ptr_vel, float* ptr_coeff,
	...
	size_t n1_Tblock, size_t n2_Tblock, size_t n3_Tblock, size_t end_z,
    ...
    {		
		...
		q.submit([&](handler &cgh) {
			...
			auto local_nd_range = range<3>(n1_Tblock, n2_Tblock, 1);
			auto global_nd_range = range<3>((n1-2*HALF_LENGTH), (n2-2*HALF_LENGTH), (n3-2*HALF_LENGTH)/n3_Tblock);
	
#ifdef USE_SHARED
			auto localRange_ptr_prev = range<1>((n1_Tblock + (2 * HALF_LENGTH) + PAD) * (n2_Tblock + (2 * HALF_LENGTH)));
