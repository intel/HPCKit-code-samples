...
void iso_3dfd_iteration_cpu(cl::sycl::nd_item<3> it, float* next, float* prev,
                            float* vel, const float* c, size_t nx, size_t nxy,
                            size_t bx, size_t by, size_t z_offset,
                            size_t full_end_z) {
...
#pragma unroll(HALF_LENGTH)
    for (size_t iter = 1; iter <= HALF_LENGTH; iter++) {
      value += c[iter] * (prev[gid + iter] + prev[gid - iter] +
 ...
 *
 */
bool iso_3dfd_cpu(cl::sycl::queue& q, float* ptr_next, float* ptr_prev,
        ...
        cgh.parallel_for<class iso_3dfd_kernel_cpu>(
            nd_range<3>{global_nd_range, local_nd_range}, [=](nd_item<3> it) {
              iso_3dfd_iteration_cpu(it, next.get_pointer(), prev.get_pointer(),
                                     vel.get_pointer(), coeff.get_pointer(), nx,
                                     nxy, bx, by, n3_Tblock, end_z);
            });
      else
