  ...
  if (sycl) {
    ...
    */
    ...
    if (isGPU) {
      ...
      queue q(cpu_sel, exception_handler);
      ...
      iso_3dfd_cpu(q, next_base, prev_base, vel_base, coeff, n1, n2, n3,
                   n1_Tblock, n2_Tblock, n3_Tblock, n3 - HALF_LENGTH,
                   nIterations);
      q.wait_and_throw();
    }
    ...
    if (omp && sycl) {
      if (nIterations % 2) {
        error =
            within_epsilon(next_base, temp, n1, n2, n3, HALF_LENGTH, 0, 0.1f);
        if (error) std::cout << "Error  = " << error << std::endl;
      } else {
        error =
            within_epsilon(prev_base, temp, n1, n2, n3, HALF_LENGTH, 0, 0.1f);
        if (error) std::cout << "Error  = " << error << std::endl;
      }
      delete[] temp;
    }
