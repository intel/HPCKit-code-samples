 ...
 * also to compare OpenMP performance on CPU with the SYCL Kernel
 */
...
int main(int argc, char* argv[]) {
	...
	for (unsigned int arg = 8; arg < argc; arg++) {
		...
		else if (std::string(argv[arg]) == "sycl" || std::string(argv[arg]) == "SYCL") {
			omp = false;
			sycl = true;
		}
		else if (std::string(argv[arg]) == "gpu" || std::string(argv[arg]) == "GPU") {
			isGPU = true;
		}
		else if (std::string(argv[arg]) == "cpu" || std::string(argv[arg]) == "CPU") {
			isGPU = false;
		}
		else {
			usage(argv[0]);
	...
	if (sycl) {
		...
		if (isGPU)
		{
			...
			//iso_3dfd_device(q, next_base, prev_base, vel_base, coeff, n1, n2, n3, HALF_LENGTH, HALF_LENGTH, HALF_LENGTH, n3 - HALF_LENGTH, nIterations);
			iso_3dfd_device(q, next_base, prev_base, vel_base, coeff, n1, n2, n3, n1_Tblock, n2_Tblock, n3_Tblock, n3 - HALF_LENGTH, nIterations);
			q.wait_and_throw();
		}
		else {
			// Skipping GPU if there are any
			std::string pattern("CPU");
			MyDeviceSelector cpu_sel(pattern);

			queue q(cpu_sel, [&](exception_list eL) {
				try {
					for (auto& e : eL) {
						std::rethrow_exception(e);
					}
			...
			//iso_3dfd_cpu(q, next_base, prev_base, vel_base, coeff, n1, n2, n3, HALF_LENGTH, HALF_LENGTH, HALF_LENGTH, n3 - HALF_LENGTH, nIterations);
			iso_3dfd_cpu(q, next_base, prev_base, vel_base, coeff, n1, n2, n3, n1_Tblock, n2_Tblock, n3_Tblock, n3 - HALF_LENGTH, nIterations);
			q.wait_and_throw();
		}
